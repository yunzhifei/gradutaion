...
public interface IHotelOderDao {
    ...
    @Select({"select " + SELECT_FIELD + " from " + TABLE_NAME + " where payState=1 " + " and userId=#{userId} limit #{offset},#{rows}"})
    ...
    @Select({"select count(1) " + " from " + TABLE_NAME + " where payState=1 and " + "userId=#{userId}"})
